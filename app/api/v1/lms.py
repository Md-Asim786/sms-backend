...
def get_teacher_class_subject_ids(
...
def verify_teacher_teaches_subject(
    db: Session, teacher_id: uuid.UUID, class_subject_id: uuid.UUID
) -> bool:
    """Verify that a teacher teaches a specific subject"""
    # First check if teacher_id is a User ID, if so get the EnrolledEmployee ID
    from app.models.users import EnrolledEmployee

    employee = (
        db.query(EnrolledEmployee)
        .filter(EnrolledEmployee.user_id == teacher_id)
        .first()
    )
    if employee:
...
def verify_student_enrolled(
    db: Session, student_id: uuid.UUID, class_subject_id: uuid.UUID
) -> bool:
...
def mark_bulk_attendance(
    ...
    if (
        not verify_teacher_teaches_subject(
            db, current_user.id, request.class_subject_id
        )
...
def get_class_attendance(
    ...
    query = db.query(AttendanceRecord).filter(
        AttendanceRecord.class_subject_id == class_subject_id
    )
...
def update_attendance(
    ...
    new_status = update_data.status.value if update_data.status else old_status
    ...
    # Log attendance edit
    audit_logger.log_attendance_edited(
...
def create_assignment(
    ...
    teacher_subject = (
        db.query(TeacherSubject).filter(TeacherSubject.id == class_subject_id).first()
    )
    ...
    # Verify teacher teaches this subject
    if (
        not verify_teacher_teaches_subject(db, current_user.id, actual_class_subject_id)
        and current_user.role != "admin"
    ):
        return {
            "success": False,
...
def list_assignments(
    ...
    """List assignments based on user role with pagination"""
    ...
    if current_user.role == "student":
        ...
        class_subject_ids = (
            get_teacher_class_subject_ids(db, current_user.id)
...
def get_assignment(
    ...
    # Check access
    if current_user.role == "student":
        if not verify_student_enrolled(
            db, current_user.id, assignment.class_subject_id
        ):
            return {
                "success": False,
...
def delete_assignment(
    ...
    audit_logger.log_assignment_deleted(
    ...
    return {"success": True, "message": "Assignment deleted successfully", "data": None}
...
def submit_assignment(
    ...
    if (
        assignment.due_date
        and datetime.now(assignment.due_date.tzinfo) > assignment.due_date
    ):
        return {
            "success": False,
...
def delete_submission(
    ...
    if not submission:
        return {"success": False, "message": "No submission found", "data": None}

    db.delete(submission)
    db.commit()

    return {"success": True, "message": "Submission deleted successfully", "data": None}
...
def grade_submission(
    ...
    if not submission:
        return {"success": False, "message": "Submission not found", "data": None}

    # Store old grade for audit
    ...
    # Log grade update
    audit_logger.log_grade_updated(
...
@router.get("/grades/student", response_model=APIResponse)
...
@router.get("/grades/class/{class_subject_id}", response_model=APIResponse)
def get_class_grades(
    class_subject_id: uuid.UUID,
    db: Session = Depends(deps.get_db),
    current_user: User = Depends(deps.RoleChecker(["teacher", "admin"])),
):
    """Get all grades for a specific subject (Teacher view)"""
    # Verify teacher teaches this subject
    if (
        not verify_teacher_teaches_subject(db, current_user.id, class_subject_id)
        and current_user.role != "admin"
    ):
        return {
            "success": False,
    ...
    assignments = (
        db.query(Assignment)
        .filter(Assignment.class_subject_id == class_subject_id)
        .all()
    )
    ...
    for assignment in assignments:
        ...
        for sub in submissions:
            student = (
                ...
                .filter(EnrolledStudent.id == sub.student_id)
                .first()
            )
            result.append(
                {
                    ...
                    "student_roll_number": student.admission_number
                    if student
                    else None,
                    "grade": sub.grade,
                    "feedback": sub.feedback,
...
def create_lecture(
    ...
    print(f"Creating lecture - User: {current_user.id}, Role: {current_user.role}")
    print(f"Class subject ID: {lecture_in.class_subject_id}")

    # Verify teacher teaches this subject
    is_teaching = verify_teacher_teaches_subject(
        db, current_user.id, lecture_in.class_subject_id
    )
    print(f"Is teaching: {is_teaching}")

    if not is_teaching and current_user.role != "admin":
        return {
            "success": False,
            "message": "You do not teach this subject",
            "data": None,
    ...
    lecture_data = lecture_in.model_dump()
    lecture_data["author_id"] = current_user.id
    ...
    if hasattr(lecture_type, "value"):
        lecture_type = lecture_type.value
    elif lecture_type:
        lecture_type = str(lecture_type)
    else:
        lecture_type = "other"

    lecture_dict = {
        "id": str(lecture.id),
        "class_subject_id": str(lecture.class_subject_id),
        "title": lecture.title,
        "description": lecture.description or "",
        "content_url": lecture.content_url or "",
        "type": lecture_type,
        "is_published": lecture.is_published,
        "author_id": str(lecture.author_id) if lecture.author_id else None,
        "view_count": lecture.view_count,
        "download_count": lecture.download_count,
        "scheduled_at": lecture.scheduled_at.isoformat()
...
@router.get("/lectures", response_model=PaginatedResponse)
def list_lectures(
    class_subject_id: Optional[uuid.UUID] = None,
    ...
    """List lectures based on user role with pagination"""
    ...
    offset = (page - 1) * limit
    lectures = (
        query.order_by(Lecture.scheduled_at.desc(), Lecture.created_at.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def get_lecture(
    ...
    if not lecture:
        return {"success": False, "message": "Lecture not found", "data": None}
...
def update_lecture(
    ...
    return {"success": True, "message": "Lecture updated successfully", "data": lecture}
...
def delete_lecture(
    ...
    audit_logger.log_lecture_deleted(
    ...
    return {"success": True, "message": "Lecture deleted successfully", "data": None}
...
def get_my_subjects(
    ...
    return {"success": False, "message": "Invalid role", "data": None}
        ...
        result.append(
            {
                ...
                "receiver_name": msg.receiver.email if msg.receiver else "Unknown",
                "subject": msg.subject,
                "content": msg.content,
                "is_read": msg.is_read,
                "is_sent": is_sender,
...
def send_message(
    ...
    if not receiver:
        return {"success": False, "message": "Receiver not found", "data": None}
...
@router.put("/messages/{message_id}/read", response_model=APIResponse)
        ...
        notifications.append(
            {
                ...
                "from": msg.sender.email if msg.sender else "Unknown",
