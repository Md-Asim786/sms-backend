...
def set_config(config: schemas.TimetableConfigCreate, db: Session = Depends(get_db)):
    ...
    db_config = (
        db.query(models.TimetableConfig)
        .filter(models.TimetableConfig.academic_year_id == config.academic_year_id)
        .first()
    )
...
def update_config(
    config_id: UUID,
    config_update: schemas.TimetableConfigUpdate,
    db: Session = Depends(get_db),
):
    db_config = (
        db.query(models.TimetableConfig)
        .filter(models.TimetableConfig.id == config_id)
        .first()
    )
    if not db_config:
...
def get_config(academic_year_id: UUID, db: Session = Depends(get_db)):
    config = (
        db.query(models.TimetableConfig)
        .filter(models.TimetableConfig.academic_year_id == academic_year_id)
        .order_by(models.TimetableConfig.created_at.desc())
        .first()
    )
...
@router.get(
    "/versions/{academic_year_id}", response_model=List[schemas.TimetableVersion]
)
def get_versions(academic_year_id: UUID, db: Session = Depends(get_db)):
    return (
        db.query(models.TimetableVersion)
        .filter(models.TimetableVersion.academic_year_id == academic_year_id)
        .order_by(models.TimetableVersion.created_at.desc())
        .all()
    )


@router.patch(
    "/versions/{version_id}/activate", response_model=schemas.TimetableVersion
)
def activate_version(version_id: UUID, db: Session = Depends(get_db)):
    version = (
        db.query(models.TimetableVersion)
        .filter(models.TimetableVersion.id == version_id)
        .first()
    )
    if not version:
...
def set_teacher_constraint(
    constraint: schemas.TeacherConstraintCreate, db: Session = Depends(get_db)
):
    # Update if exists, else create
    db_constraint = (
        db.query(models.TeacherConstraint)
        .filter(
...
def set_subject_constraint(
    constraint: schemas.SubjectConstraintCreate, db: Session = Depends(get_db)
):
    db_constraint = (
        db.query(models.SubjectConstraint)
        .filter(
...
def generate_timetable(
    request: schemas.TimetableGenerationRequest, db: Session = Depends(get_db)
):
    try:
        ...
        if not version:
            raise HTTPException(
                status_code=400,
                detail="Could not generate any slots. Please check if subjects and teachers are correctly assigned.",
            )
        return version
...
def get_class_timetable(
    ...
    if version_id:
        ...
        active_version = (
            db.query(models.TimetableVersion)
            .filter(models.TimetableVersion.is_active == True)
            .first()
        )
        if active_version:
    ...
    return query.order_by(
        models.TimetableSlot.day, models.TimetableSlot.period_index
    ).all()
...
def get_my_timetable(
    ...
    """Get current user's (teacher/student) timetable"""
    from app.models.users import EnrolledEmployee, EnrolledStudent
    ...
    if not user_id:
        return {"success": False, "message": "User profile not found", "data": []}

    # Get timetable slots based on role
    if current_user.role == "teacher":
        query = db.query(models.TimetableSlot).filter(
            models.TimetableSlot.teacher_id == user_id
        )
    elif current_user.role == "student":
        query = db.query(models.TimetableSlot).filter(
            models.TimetableSlot.class_id
            == (
                db.query(EnrolledStudent).filter(EnrolledStudent.id == user_id).first()
            ).class_id
            if db.query(EnrolledStudent).filter(EnrolledStudent.id == user_id).first()
            else None
        )
    else:
    ...
    if version_id:
        ...
        if active_version:
            ...
            return {"success": True, "message": "No active timetable", "data": []}
...
def get_teacher_schedule(
    teacher_id: UUID, version_id: Optional[UUID] = None, db: Session = Depends(get_db)
):
    query = db.query(models.TimetableSlot).filter(
...
@router.get(
    "/teacher/{teacher_id}/constraint",
    response_model=Optional[schemas.TeacherConstraint],
)
def get_teacher_constraint(
    teacher_id: UUID, academic_year_id: UUID, db: Session = Depends(get_db)
):
    return (
        db.query(models.TeacherConstraint)
        .filter(
...
def delete_version(version_id: UUID, db: Session = Depends(get_db)):
    ...
    if version.is_active:
        raise HTTPException(
            status_code=400,
            detail="Cannot delete the active version. Deactivate it first by activating another version.",
        )
